  ...
  methods: {
    ...
    onAddPeer(config) {
      ...
      dataChannels[peer_id] =
        peerConnection.createDataChannel("talk_data_channel");
    ...
    handleOnIceCandidate: (peer_id, meeting_id, sender, stompClient) => {
      peers[peer_id].onicecandidate = ({ candidate }) => {
    ...
    handleICECandidate: function (config) {
      ...
      peers[config.peerId]
        .addIceCandidate(new RTCIceCandidate(config.iceCandidate))
        .catch((err) => {
    ...
    init() {
      ...
      this.setRoomLink(
        this.getAppUrl() + "/teams/meeting/" + this.getMeetingId
      );
      this.connect();
