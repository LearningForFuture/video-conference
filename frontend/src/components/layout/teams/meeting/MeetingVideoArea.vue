...
export default {
  ...
  watch: {
    getAudioEnabled(audioEnabled) {
      if (audioEnabled) {
        document
          .getElementById(this.getSender)
          .querySelector(".wrap-status-video svg.bi-mic-mute-fill")
          .setAttribute("class", "bi-mic-fill");
        let path = document
          .getElementById(this.getSender)
          .querySelectorAll(".wrap-status-video svg.bi-mic-fill path");
        path[0].setAttribute("d", "M5 3a3 3 0 0 1 6 0v5a3 3 0 0 1-6 0V3z");
        path[path.length - 1].setAttribute(
          "d",
        ...
        document
          .getElementById(this.getSender)
          .querySelector(".wrap-status-video svg.bi-mic-fill")
          .setAttribute("class", "bi-mic-mute-fill");
        let path = document
          .getElementById(this.getSender)
          .querySelectorAll(".wrap-status-video svg.bi-mic-mute-fill path");
        path[0].setAttribute(
          "d",
    ...
    getVideoEnabled(videoEnabled) {
      document
        .getElementById(this.getSender)
        .querySelector(".avatar-user").style.visibility = videoEnabled
        ? "hidden"
        : "visible";
    },
  ...
  methods: {
    ...
    changeMicrophone: function (deviceId) {
      navigator.mediaDevices
        ...
        .then((micStream) => {
          ...
          const newStream = new MediaStream([
            this.getLocalMediaStream.getVideoTracks()[0],
    ...
    linkify: function (str) {
      return this.sanitizeString(str).replace(
        /(?:(?:https?|ftp):\/\/)?[\w/\-?=%.]+\.[\w/\-?=%]+/gi,
        (match) => {
          let displayURL = match
            ...
            .replace("https://", "");
          displayURL =
            displayURL.length > 25
              ? displayURL.substr(0, 25) + "&hellip;"
              : displayURL;
    ...
    sendChat: function (e) {
      ...
      if (this.typing.length) {
        ...
        Object.keys(dataChannels).map((peer_id) =>
          dataChannels[peer_id].send(JSON.stringify(chatMessage))
        );
        this.typing = "";
    ...
    onConnected() {
      ...
      this.stompClient.subscribe(
        "/topic/messages/" + this.getMeetingId,
        this.onMessageReceived
      );
      this.stompClient.subscribe(
        "/user/topic/messages/" + this.getMeetingId,
        this.onMessageReceived
      );
    ...
    roomId: () => {
      let roomName = location.pathname.substring(
        location.pathname.search("meeting") + 8
      );
    ...
    setupLocalMedia(roomId, callback, errorback) {
      ...
      navigator.mediaDevices
        ...
        .then((stream) => {
          ...
          const localMedia = this.getVideoElement(
            this.getSender,
            this.getUsername,
            this.getVideoEnabled,
            true
          );
        ...
        .catch((err) => {
          ...
          console.log(
            "This site will not work without camera/microphone access."
          );
    ...
    getVideoElement: (peerId, name, videoEnabled, isLocal) => {
      ...
      const svgMic = document.createElementNS(
      ...
      const pathTopMic = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "path"
      );
      const pathTBottomMic = document.createElementNS(
        "http://www.w3.org/2000/svg",
        "path"
      );
      pathTopMic.setAttribute("d", "M5 3a3 3 0 0 1 6 0v5a3 3 0 0 1-6 0V3z");
      pathTopMic.setAttribute("color", "#fff");
      pathTBottomMic.setAttribute(
        "d",
        "M3.5 6.5A.5.5 0 0 1 4 7v1a4 4 0 0 0 8 0V7a.5.5 0 0 1 1 0v1a5 5 0 0 1-4.5 4.975V15h3a.5.5 0 0 1 0 1h-7a.5.5 0 0 1 0-1h3v-2.025A5 5 0 0 1 3 8V7a.5.5 0 0 1 .5-.5z"
      );
      pathTBottomMic.setAttribute("color", "#fff");
      svgMic.appendChild(pathTopMic);
      svgMic.appendChild(pathTBottomMic);
      ...
      image.src =
        "https://eu.ui-avatars.com/api/?size=200&background=random&rounded=true&name=" +
        name.replace(" ", "%");
    ...
    resizeVideos: () => {
      ...
      const mh =
        window.innerHeight -
        document.querySelector(".wrapper-header").offsetHeight -
        document.querySelector(".container-video-call").offsetHeight;
      let width =
        len === 1
          ? mw
          : len > 2 && len <= 4
    ...
    handleOnIceCandidate: (peer_id, meeting_id, sender, stompClient) => {
      store.state.meeting.peers[peer_id].onicecandidate = ({ candidate }) => {
          ...
          try {
            ...
            switch (msg.type) {
              ...
              case "refreshStatusAudio":
                if (msg.audioEnabled) {
                  document
                    .getElementById(msg.peer_id)
                    .querySelector(".wrap-status-video svg.bi-mic-mute-fill")
                    .setAttribute("class", "bi-mic-fill");
                  let path = document
                    .getElementById(msg.peer_id)
                    .querySelectorAll(
                      ".wrap-status-video svg.bi-mic-fill path"
                    );
                  path[0].setAttribute(
                    "d",
                    "M5 3a3 3 0 0 1 6 0v5a3 3 0 0 1-6 0V3z"
                  );
                  path[path.length - 1].setAttribute(
                    "d",
                    "M3.5 6.5A.5.5 0 0 1 4 7v1a4 4 0 0 0 8 0V7a.5.5 0 0 1 1 0v1a5 5 0 0 1-4.5 4.975V15h3a.5.5 0 0 1 0 1h-7a.5.5 0 0 1 0-1h3v-2.025A5 5 0 0 1 3 8V7a.5.5 0 0 1 .5-.5z"
                  ...
                  let path = document
                    .getElementById(msg.peer_id)
                    .querySelectorAll(
                      ".wrap-status-video svg.bi-mic-mute-fill path"
                    );
                  path[0].setAttribute(
                    "d",
                    "M13 8c0 .564-.094 1.107-.266 1.613l-.814-.814A4.02 4.02 0 0 0 12 8V7a.5.5 0 0 1 1 0v1zm-5 4c.818 0 1.578-.245 2.212-.667l.718.719a4.973 4.973 0 0 1-2.43.923V15h3a.5.5 0 0 1 0 1h-7a.5.5 0 0 1 0-1h3v-2.025A5 5 0 0 1 3 8V7a.5.5 0 0 1 1 0v1a4 4 0 0 0 4 4zm3-9v4.879L5.158 2.037A3.001 3.001 0 0 1 11 3z"
              ...
              case "refreshStatusVideo":
                document
                  .getElementById(msg.peer_id)
                  .querySelector(".avatar-user").style.visibility =
                  msg.videoEnabled ? "hidden" : "visible";
                break;
              default:
                break;
            }
    ...
    handleRTCOffer: (peer_id, meeting_id, sender, stompClient) => {
      store.state.meeting.peers[peer_id]
        .createOffer()
    ...
    loadRemoteMediaStream(stream, peer_id, peerName) {
      ...
      const remoteMedia = this.getVideoElement(
        peer_id,
        peerName,
        this.getVideoEnabled
      );
    ...
    async init() {
      try {
        ...
        if (
          ...
          this.$toast.error(
            "Access is not permitted! Please try logging in again later!",
            {
              position: "top-center",
              timeout: 5000,
              closeOnClick: true,
