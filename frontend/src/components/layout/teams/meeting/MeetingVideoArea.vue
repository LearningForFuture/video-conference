...
export default {
    ...
    methods: {
        ...
        handleIncomingDataChannelMessage: function (msg) {
            switch (msg.type) {
            case "chat":
                this.showChat = true;
            ...
            case "refreshStatusAudio":
                console.log(msg);
                // document.getElementById(msg.peer_id).c
                break;
            default:1
                break;
            }
        ...
        resizeVideos: () => {
            ...
            let width = (len === 1) ? mw : (len > 2 && len <= 6) ? mw / 3 : (len > 6 && len <= 8) ? mw / 4 : mw / 5;
            ...
            document.querySelectorAll("#videos .video").forEach((v) => {
                ...
                if (len === 2)  v.style.height = "100%";
                else v.style.height = height + "px";
            });
        },
